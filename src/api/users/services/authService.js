...
const login = async ({ username, password }) => {
  try {
    ...
    // Check if account is locked and exit login process immediately
    const currentTime = new Date();
    const accountIsLocked = user.status === 'locked';
    const lockoutDurationHasNotPassedYet =
      user.lockout_duration !== null && user.lockout_duration > currentTime ? true : false;
    if (accountIsLocked && lockoutDurationHasNotPassedYet) {
      throw new UnauthorizedError(
    ...
    if (!passwordIsCorrect) {
      // Record failed login attempt, check if it exceeds allowed attempts and lock account accordingly
      await UserRepository.recordLoginAttempts(username);
      const updatedUser = await UserRepository.findUser(username);
      const maximumAllowedLoginAttempts = 3;
      const loginAttemptsAreGreaterThanAllowed = updatedUser.login_attempts >= maximumAllowedLoginAttempts;
      if (loginAttemptsAreGreaterThanAllowed) {
        await UserRepository.lockAccount({
          username: username,
          lockoutDuration: config.authentication.loginRetryWindow,
        });
        throw new UnauthorizedError(
          'This account is temporarily locked due to too many failed login attempts. Please wait a few moments then try again.',
        );
    ...
    // If login is successful, reset the login attempts and unlock account
    await UserRepository.clearLoginAttempts(username);
    await UserRepository.unlockAccount(username);
